...
function AnalyticsSkeleton() {
  return (
    <div className="space-y-4 animate-pulse">
      <div style={{ height: 32, width: 220, borderRadius: 10, background: "var(--surface-3)" }} />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} style={{ height: 96, borderRadius: 14, background: "var(--surface-3)" }} />
        ))}
      </div>
      <div style={{ height: 260, borderRadius: 14, background: "var(--surface-3)" }} />
      <div style={{ height: 200, borderRadius: 14, background: "var(--surface-3)" }} />
    </div>
  );
}
...
async function getAnalyticsData(userId: string) {
  ...
  const xpFromWords = savedWords * 2;
  ...
  return {
    overview: {
      ...
      consistencyPct,
      avgWeeklyXp,
      memberSince: (user?.createdAt ?? now).toISOString(),
    ...
    vocabulary: {
      ...
      recentWords: savedWordsWithWord.slice(0, 10).map((sw) => ({
        ...
        addedAt: sw.addedAt.toISOString(),
      })),
    },
    stories: {
